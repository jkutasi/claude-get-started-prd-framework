...
PHASE B: GHERKIN AUDIT + TEST SPECIFICATION + TEST PEER REVIEW (Article 17, 18)
   ...
   16. QA Lead audits Gherkin for completeness (traceability matrix) + quality
   17. Every user story element must map to at least one Gherkin scenario
   18. Quality check: unambiguous, concrete values, testable outcomes, NFR coverage
   19. If gaps: write missing Gherkin, re-audit (max 3 cycles, then owner sign-off)

   B.2: TEST SPECIFICATION (different agents from implementation coders)
   20. Architect defines skeletal interfaces (function sigs, class stubs, type stubs)
   21. QA Lead spawns test-writer sub-agents (NOT implementation coders)
   22. Test-writers write ALL tests: unit, integration, E2E definitions
   23. ALL tests must be RED (import errors or assertion failures)
   24. Tests that PASS = bad test, must be fixed

   B.3: TEST PEER REVIEW (3+ models, parallel)
   25. 3 peer reviewers review test code in parallel (same process as code review)
   26. Consensus issues (2+ reviewers) = mandatory test fixes
   27. Fixed tests re-validated: still RED against skeletal interfaces
...
28. CTO assigns implementation coder teammates with focused module scope
29. Coders receive failing tests + spec, write code until all tests PASS
30. Coders do NOT modify tests (only implementation code)
...
31. Each coder re-reads their code, identifies issues, proposes improvements
32. CTO reviews reflection, assigns self-identified fixes
...
33. 3 peer reviewers run in parallel, return findings
...
34. CTO synthesizes: consensus issues (2+ reviewers) = mandatory fixes

PHASE F: QA SWARM + WHISKEY TEAM + UX SENSE CHECK (AUTONOMOUS FIX)
35. Standard QA swarm runs in parallel (red team framing -- Article 7c):
...
36. Whiskey Team adversarial QA runs (all 8 test categories -- Article 15)
    - Whiskey Team applies same autonomous fix protocol for all findings
37. Implicit Behavior Regression check runs (all 6 categories -- Article 15b)
38. UX Sense Check runs via agent-browser with all personas
    (Article 16 -- frontend slices only)
39. QA Manager synthesizes all findings + autonomous fix results into report
...
44. CTO reviews autonomous fix results from Phase F:
...
45. Escalated fixes go through abbreviated peer review
46. Red Team Post-QA review runs (Article 14b):
...
47. If Red Team issues BLOCK: escalate to project owner
48. Autonomous Defect Resolution Protocol (Article 17e):
...
49. Abbreviated QA re-run on fixed areas only
...
50. Implicit Behavior Regression re-check (all 6 categories)
51. Goal Achievement Test re-run if any fixes touched user-facing workflows
...
52. CTO delegates doc updates to Documentation Scribe (if available) or Architect
53. Designated agent updates affected docs via DOCS_MAP
54. If a discovery in this slice invalidates earlier diagrams, update them here

PHASE I.5: USER DELIVERY (only after ALL prior phases complete)
55. CTO presents completed slice to user:
...
56. User tests and provides feedback
57. If user finds issues: CTO spawns fix agents, runs abbreviated QA, then re-presents
...
58. CTO runs the gate check script:
    $ python gate_check.py --slice N
59. Script mechanically verifies ALL artifacts exist on disk:
...
62. If PASS: push to GitHub, then run POST-PUSH VERIFICATION.
